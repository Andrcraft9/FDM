program CSR
	...
	! Local 
     integer*4 :: N, i, j, nz
	real*8 :: c_error_system, l_error_system
    ...
    ! Print matrix
	call draw_matrix(N*N, IA, JA, "matrix.ps")
	...
	call ilu0(N*N, a, ja, ia, rW(ipaLU),iW(ipjLU),iW(ipjU),iW(ipiw),ierr)
	...
	call dcopy(N*N, 0d0, 0, c, 1)

    ! compute initial residual norm
	resinit = ddot(N*N, f, 1, f, 1)
	...
	iprevec(1) = N*N ! single entry required: system size 
	imatvec(1) = N*N ! single entry required: system size 
	...
	!print *, 'error C is ', c_error_system
	
	l_error_system = L2_check_err(N, c)
	!print *, 'error L2 is ', l_error_system

	print *, c_error_system, ' & ', l_error_system, ' \\ '
